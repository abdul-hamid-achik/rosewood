---
title: Configuration
description: User settings (config.toml) and per-project settings + debug configs (.rosewood.toml)
---
...
- `preLaunchTask` runs before the session starts (e.g. a build), and its result is reported in the
  debug console.
- See [Dependencies → Debugging](/docs/guides/dependencies/#debugging-lldb-dap) for the adapter requirement,
  and [Features → Debugging](/docs/guides/features/#debugging-dap-needs-lldb-dap) for how to drive a session.
