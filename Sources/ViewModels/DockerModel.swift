...
/// Owns Docker UI state, extracted from ProjectViewModel so Docker polling/refreshes only
/// re-render the Docker views instead of every view observing the app-wide view model.
/// Injected as its own @EnvironmentObject (same pattern as commandPaletteViewModel).
@MainActor
